...
server.get("/classifications/:id/codes.csv", (req, res) => {
	const mockData = require("./mock_data/changes");
	res.send(mockData);
});
...
server.get("/classifications/:id/changes", (req, res) => {
	let mockData;
	try {
		const filePath = path.resolve(MOCK_DATA_ROOT, "changes", req.params.id);
...
server.get("/ssbsections", (req, res) => {
	const mockData = require("./mock_data/ssbsections");
	res.send(mockData);
});

server.get("/classificationfamilies", (req, res) => {
	const mockData = require("./mock_data/classificationfamilies");
	res.send(mockData);
});
...
server.get("/classificationfamilies/:id", (req, res) => {
	...
	try {
		mockData = require(
			"./mock_data/classificationfamilies/classificationfamily_" +
				req.params.id,
		);
	} catch (ex) {
...
server.get("/classifications", (req, res) => {
	const mockData = require("./mock_data/classifications");
	res.send(mockData);
});
...
server.get("/classifications/:id", (req, res) => {
	...
	try {
		mockData = require(
			"./mock_data/classifications/classification_" + req.params.id,
		);
	} catch (ex) {
...
server.get("/correspondencetables/:id", (req, res) => {
	...
	try {
		mockData = require(
			"./mock_data/correspondences/correspondence_" + req.params.id,
		);
	} catch (ex) {
