...
function loadData(props, selectedLanguage) {
	...
	actions.getClassification(params.classId).then((res) => {
		...
		if (!_.isEmpty(classification) && !_.isEmpty(classification.versions)) {
			...
			_.forEach(classification.versions, (v) => {
				if (v.id == versionId) {
...
class ClassItemPage extends Component {
	...
	showFullDescription(ev) {
		ReactDOM.findDOMNode(this.refs.description).setAttribute(
			"class",
			"description",
		);
		ReactDOM.findDOMNode(this.refs.descLink).setAttribute("class", "hide");
		ReactDOM.findDOMNode(this.refs.shortDescLink).setAttribute(
			"class",
			"clickable",
		);
	}
	...
	render() {
		const { classification, selectedVersion, isFetching, actions, params } =
			this.props;
		...
		document.title =
			classification.name + counterpart.translate("PAGE.TITLE_POSTFIX");
			...
			<div className="content klass-item">
				<div className="main">
					<div className="heading">
						<p>
							<Translate content="CLASSIFICATIONS.STATISTICAL_UNIT" />:{" "}
							<b>{classification.statisticalUnits[0]}</b>
						</p>
						<h1>{classification.name}</h1>
						<p className="description short" ref="description">
							{classification.description}
						</p>
						<p
							className="clickable"
							ref="descLink"
							onClick={(ev) => this.showFullDescription(ev)}
						>
							+
							<Translate content="CLASS_ITEM.READ_MORE" />
						</p>
						<p
							className="hide"
							ref="shortDescLink"
							onClick={(ev) => this.hideFullDescription(ev)}
						>
							-
							<Translate content="CLASS_ITEM.READ_LESS" />
						</p>
					</div>
					{this.renderTabs()}
				</div>
