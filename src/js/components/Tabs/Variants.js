...
class Variants extends Component {
	...
	renderBody() {
		...
		return version.classificationVariants.map(
			((variant, key) => (
				<tr key={key}>
					<td>
						<Link to={`${getVariantPath(params, variant)}`}>
							{variant.name}
						</Link>
					</td>
	...
	render() {
		...
		if (params.itemId) {
			...
			const joinedLanguages = selectedVariant.published.map((val) => {
				const comma =
					val == selectedVariant.published[selectedVariant.published.length - 1]
						? ""
						: ",";
				if (val == "nb")
					return (
