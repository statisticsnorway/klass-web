...
class Correspondences extends Component {
							...
							<td>
								{correspondence.sourceLevel == undefined
									? counterpart.translate(
											"TABS.CORRESPONDENCES.CORRESPONDENCES_LEVELS_ALL",
										)
	...
	renderCorrTableBody(corrArr) {
		...
		switch (this.state.invertedTable) {
			case false:
				...
				groupedArray = groupBy(corrArr, (item) => [item.targetCode]);

				return groupedArray.map((item, key) => {
					const targetList = item
						.sort((a, b) => a.sourceCode - b.sourceCode)
						.map((subItem, key) => (
							<li key={key}>
			...
			case true:
				...
				groupedArray = groupBy(corrArr, (item) => [item.sourceCode]);

				return groupedArray.map((item, key) => {
					const targetList = item
						.sort((a, b) => a.targetCode - b.targetCode)
						.map((subItem, key) => (
							<li key={key}>
	...
	downloadCodes() {
		...
		const languageArgument = language == null ? "" : "?language=" + language;
		const csvURL =
			config.API_BASE_URL +
	...
	render() {
		...
		if (params.itemId) {
			...
			const joinedLanguages = selectedCorrespondence.published.map((val) => {
				const comma =
					val ==
			...
			return (
				<div>
					...
					<p>
						...
						<Translate content="TABS.CORRESPONDENCES.SECTION_FOR" />{" "}
